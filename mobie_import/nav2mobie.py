...
def nav2mobie(navfile):
    ...
    ds = mobie.metadata.read_dataset_metadata(os.path.join(mobie_root, 'data', dataset))
    ...
    for idx, item in enumerate(allitems[2:]):
        if item['Type'][0] == '2':  # item is a map
            if not skipmaps:
                ...
                if itemname in ds['sources'].keys():
                    continue
                ...
                if 'Imported' in item.keys():
                    ...
                    if item['MapMinMaxScale'] == ['0', '0'] and mergemap['im'].ndim > 2:
                        ...
                        for chidx, ch in enumerate(['R', 'G', 'B']):
                            if itemname + '_' + ch in ds['sources'].keys():
                                continue
                            ...
                            if data0.max() == 0:  # ignore empty images
                                ...
                                mobie.add_image(input_path=data0,
                                                input_key='',
                        ...
                        tforms = [mobie.metadata.get_affine_source_transform([itemname], tf_sc, name='MapScaleMat'),
                                  mobie.metadata.get_affine_source_transform([itemname], tf_tr, name='Translation')]
